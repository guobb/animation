...
function repeat() {
    var repeatAnimation = animation().loadImage(images).changPosition($rabbit1, rightRunningMap, images[0]).repeatForever();
...
function win() {
    var winAnimation = animation().loadImage(images).changPosition
    ($rabbit3, rabbitWinMap, images[2]).repeat(3).then(function () {
        console.log('win animation repeat 3 times ');
    });
...
function lose() {
    var loseAnimation = animation().loadImage(images).changPosition
    ($rabbit4, rabbitLoseMap, images[1]).wait(1000).repeat(1).then(
        function () {
            console.log('lose animation repeat');
        }
    );
    loseAnimation.start();
}
...
function run() {
    ...
    var runAnimation = animation().loadImage(images).enterFrame(function
        (success, time) {
    ...
    }).repeat(1).wait(1000).changPosition($rabbit2, rabbitWinMap, images[2]).then(
        function () {
